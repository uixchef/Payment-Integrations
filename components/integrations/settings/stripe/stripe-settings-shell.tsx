...
function SubHeader({
  ...
  const setAsDefaultDisabled =
    !activeAccountConnected || activeIsDefaultAccount
  const setAsDefaultReason = !activeAccountConnected
    ? "Connect this account to set it as the default provider."
    : activeIsDefaultAccount
      ? `This is already the default account for ${item.name}.`
      : undefined
      ...
      {showAccountTabs ? (
        <div className="flex h-[62px] min-w-0 flex-1 items-center gap-2">
          ...
          <button
            ...
            className={cn(
              "inline-flex h-9 shrink-0 cursor-pointer items-center gap-2 rounded-[8px] px-3.5 py-2",
              "font-[family-name:var(--font-inter)] text-base font-semibold leading-6 text-[#344054]",
              "outline-none transition-colors hover:bg-[#f9fafb] focus-visible:ring-2 focus-visible:ring-[#84adff]"
            )}
          >
          ...
          <div className="flex shrink-0 items-center gap-2">
            <Button
              ...
              aria-disabled={setAsDefaultDisabled}
              onClick={onSetAsDefault}
              title={setAsDefaultReason}
              className="px-2.5"
            >
              Set as default
            </Button>
