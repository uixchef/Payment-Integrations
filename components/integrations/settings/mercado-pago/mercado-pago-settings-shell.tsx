...
const MERCADO_PAGO_DOCS =
  "https://www.mercadopago.com/developers/en/docs/your-integrations/credentials"

export function MercadoPagoSettingsShell({ item }: { item: IntegrationItem }) {
  const {
  ...
  const [form, setForm] = useState<MercadoPagoFormState>(() =>
    isConnectedFromStatus("mercado-pago")
      ? SEEDED_MERCADO_PAGO
      : INITIAL_MERCADO_PAGO
  )
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-[12px] bg-white shadow-[0px_12px_16px_-4px_rgba(16,24,40,0.08),0px_4px_6px_-2px_rgba(16,24,40,0.03)]">
          <div className="flex min-h-0 flex-1 overflow-y-auto p-6">
            <div className="mx-auto flex w-full max-w-[1180px] gap-10">
              <div className="flex w-full max-w-[756px] min-w-0 flex-1 flex-col gap-6">
                ...
                {isConnected ? (
                  <SettingsCard>
                    ...
                    <SettingsActionRow
                      title="Disconnect this Mercado Pago account"
              ...
              <QuickStartGuide
                providerId={item.id}
                documentationHref={MERCADO_PAGO_DOCS}
                isConnected={isConnected}
              />
            </div>
          </div>
